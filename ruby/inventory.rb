#Homeworkd 1.4 Git, Inventory, Exercism
...
class Inventory
   #make hash with :item, :count, :inventory
  ...
  def inital_items
    ...
    for n in 1..item_count
      puts "What's the name of the item category #{n}"
      @item = gets.chomp.to_i
      puts "How many #{item} are in the inventory?"
  ...
  def do_next
    ...
    change_name if action == "4"
    full_inventory if action == "5"
    exit_program if action == "6"
  end

  def get_count
    puts "Which item do you want to look up its count?"
  ...
  def change_name
    puts "Which item do you want to change the name of?"
    @item = gets.chomp.to_s
    puts "What name do you want to change #{@item} to?"
  ...
  def full_inventory
    puts "your full inventory is #{@inventory}"
    do_next
  end
